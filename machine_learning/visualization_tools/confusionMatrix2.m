function C = confusionMatrix2(z1, z2, names)
...
for i=1:length(label2)
    label2{i} = sprintf('%s     ', label2{i});
end
set(ha, 'FontSize', 10)
set(ha, 'YTick', t);
set(ha, 'YTickLabel', label2(t));
set(ha, 'FontSize', 10)
set(ha, 'XTick', 0);
title(sprintf('Average Accuracy %g',  mean(C_)))
...
for i = 1:size(C,1)
%     for j = 1:size(C,2)
%         text(i,j,num2str(round(C(i,j))), 'fontsize', 10, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'middle');
%     end
end
